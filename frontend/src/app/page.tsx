...
export default function Home() {
  ...
  useEffect(() => {
    if (activeNav === "multi") {
      fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/multi/documents`)
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/documents/upload`, { method: "POST", body: form });
  ...
  const pollAnalysis = (id) => {
    const iv = setInterval(async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/analysis/${id}`);
  ...
  const loadDoc = async (doc) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/analysis/${doc.id}`);
      const data = await res.json();
  ...
  const sendChat = async () => {
    ...
    setIsThinking(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/chat/${activeDoc.id}/message`, {
  ...
  const doSemanticSearch = async () => {
    ...
    setSearchResults([]);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/analysis/search/semantic`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const doMultiAnalysis = async () => {
    ...
    setMultiResult(null);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/multi/analyze`, {
        method: "POST", headers: { "Content-Type": "application/json" },
                      ...
                      {activeTab === "summary" && (
                        <div className="content">
                          ...
                          <div className="export-row">
                            ...
                            <button className="btn btn-primary" onClick={() => window.open(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/export/${activeDoc.id}/pdf`, "_blank")}>⬇ Exporter PDF</button>
                          </div>
                        </div>
                      )}
